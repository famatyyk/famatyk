-- addons/smart_target.lua
-- "Smart Target" addon for Gift of Life.
-- Auto-selects a nearby monster if you have no target.
...
local function attack(c)
  if not (g_game and type(g_game.attack) == "function") then return end
  pcall(g_game.attack, c)
end
...
local function resolveCfg(centralCfg)
  ...
  if type(centralCfg) == "table" and type(centralCfg.addons) == "table" and type(centralCfg.addons.smart_target) == "table" then
    cfg = deepMerge(cfg, centralCfg.addons.smart_target.cfg or {})
  end
  return cfg
end
...
function A.init(entry, centralCfg)
  ...
  stopEvent()
  local tickMs = math.max(80, math.min(800, tonumber(cfg.tickMs) or 250))
  _evt = scheduleTick(function()
    ...
    local specs = getSpectatorsAround(ppos, tonumber(cfg.range) or 8)
    for _, c in ipairs(specs) do
      if (isMonster(c) or (cfg.allowPlayers == true and isPlayer(c))) then
    ...
    if best then
      if g_game then
        if type(g_game.setChaseMode) == "function" and cfg.chaseMode ~= nil then pcall(g_game.setChaseMode, tonumber(cfg.chaseMode) or 1) end
        if type(g_game.setFightMode) == "function" and cfg.fightMode ~= nil then pcall(g_game.setFightMode, tonumber(cfg.fightMode) or 2) end
      end
      attack(best)
    end
  end, tickMs)

  safePrint("[GoL][SmartTarget] init")
end

function A.shutdown()
  stopEvent()
  _loadToken = _loadToken + 1
  safePrint("[GoL][SmartTarget] shutdown")
end
